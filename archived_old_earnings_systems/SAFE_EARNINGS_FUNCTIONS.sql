-- =====================================================
-- SAFE EARNINGS FUNCTIONS - NO TABLE MODIFICATIONS
-- =====================================================
...
DROP FUNCTION IF EXISTS calculate_trend_earnings_amount(TEXT, INTEGER, INTEGER) CASCADE;
DROP FUNCTION IF EXISTS calculate_validation_earnings_amount(TEXT) CASCADE;
DROP FUNCTION IF EXISTS get_validation_rate_examples() CASCADE;
DROP FUNCTION IF EXISTS get_trend_rate_examples() CASCADE;
...
BEGIN
    ...
    SELECT 
        ...
        format('$0.02 × %s tier (%sx) = $%s', 
               t.tier_name, 
               get_tier_multiplier(t.tier_name),
               calculate_validation_earnings_amount(t.tier_name))::TEXT
    FROM (VALUES 
        ('learning'),
        ('verified'), 
        ('elite'),
...
GRANT EXECUTE ON FUNCTION calculate_trend_earnings_amount TO authenticated;
GRANT EXECUTE ON FUNCTION calculate_validation_earnings_amount TO authenticated;
GRANT EXECUTE ON FUNCTION get_validation_rate_examples TO authenticated;
GRANT EXECUTE ON FUNCTION get_trend_rate_examples TO authenticated;
...
DO $$
BEGIN
    RAISE NOTICE '✅ Safe earnings functions installed successfully!';
    RAISE NOTICE '';
    ...
    RAISE NOTICE '   Run: SELECT * FROM get_validation_rate_examples();';
    RAISE NOTICE '   Run: SELECT * FROM get_trend_rate_examples();';
    RAISE NOTICE '';
    RAISE NOTICE '✨ Example for learning tier validation:';
    RAISE NOTICE '   $0.02 × 1.0 = $0.02 per validation';
    RAISE NOTICE '';
    RAISE NOTICE '✨ Example for verified tier with streaks:';
    RAISE NOTICE '   $0.25 × 1.5 × 1.5 × 1.5 = $0.84 per trend';
END $$;

-- Show example calculations immediately
SELECT * FROM get_validation_rate_examples();
SELECT * FROM get_trend_rate_examples();