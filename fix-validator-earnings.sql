...
-- 2. Create function to add earnings for validators
CREATE OR REPLACE FUNCTION add_validator_earnings()
RETURNS TRIGGER AS $$
DECLARE
    earning_amount DECIMAL(10,2) := 0.01; -- $0.01 per validation
BEGIN
...
-- 7. Backfill earnings for existing validations (optional - run this if you want to credit past validations)
-- INSERT INTO public.earnings_ledger (user_id, trend_submission_id, amount, status, earning_type, notes, created_at)
-- SELECT 
--     validator_id,
--     trend_submission_id,
--     0.01,
--     'approved',
--     'validation',
--     'Backfilled earnings for validation: ' || vote,
--     created_at
-- FROM public.trend_validations
-- WHERE validator_id IS NOT NULL
-- ON CONFLICT DO NOTHING;

-- 8. Update all user earnings summaries
-- UPDATE public.profiles 
-- SET 
--     earnings_approved = COALESCE((
--         SELECT SUM(amount) FROM public.earnings_ledger 
...
COMMENT ON FUNCTION add_validator_earnings() IS 'Automatically adds $0.01 earnings to validators profile when they verify a trend';
