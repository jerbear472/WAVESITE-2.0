...
-- Drop existing versions and create the correct one
-- ============================================

-- Step 1: Drop ALL existing versions of the function
DROP FUNCTION IF EXISTS cast_trend_vote(UUID, UUID, BOOLEAN);
DROP FUNCTION IF EXISTS cast_trend_vote(UUID, UUID, BOOLEAN, DECIMAL);
DROP FUNCTION IF EXISTS cast_trend_vote(UUID, UUID, BOOLEAN, DECIMAL, TEXT);
DROP FUNCTION IF EXISTS cast_trend_vote(UUID, UUID, BOOLEAN, NUMERIC, TEXT);
DROP FUNCTION IF EXISTS cast_trend_vote CASCADE;

-- Step 2: Create the correct version
...
SELECT 'Function replaced successfully!' as status;