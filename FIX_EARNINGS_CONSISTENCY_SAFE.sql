-- ============================================================
-- FIX EARNINGS CONSISTENCY (SAFE VERSION)
...
DROP FUNCTION IF EXISTS get_user_earnings(UUID) CASCADE;
...
EXCEPTION WHEN OTHERS THEN
    ...
    RAISE NOTICE 'Error in earnings calculation: %', SQLERRM;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
-- STEP 6: Create or replace view for earnings leaderboard
DROP VIEW IF EXISTS earnings_leaderboard;
CREATE VIEW earnings_leaderboard AS
SELECT 
    p.id,
    p.email,
    p.username,
...
BEGIN
    ...
    RAISE NOTICE '=================================';
    RAISE NOTICE '';
    RAISE NOTICE 'Bonus Structure:';
    RAISE NOTICE '  📸 Screenshot: +$0.15';
    RAISE NOTICE '  🌊 Wave Score >70: +$0.20';
    RAISE NOTICE '  👤 Creator Info: +$0.05';
    RAISE NOTICE '  #️⃣ 3+ Hashtags: +$0.05';
    RAISE NOTICE '  📝 Caption: +$0.05';
    RAISE NOTICE '  💯 High Engagement: +$0.20';
    RAISE NOTICE '  📈 100k+ Views: +$0.25';
    RAISE NOTICE '  🔥 1M+ Views: +$0.50';
    RAISE NOTICE '=================================';
END $$;
...
SELECT 
    ...
    ROUND(AVG(COALESCE(payment_amount, 1.00)), 2) as avg_payment
FROM trend_submissions;