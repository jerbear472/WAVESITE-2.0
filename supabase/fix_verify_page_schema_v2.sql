-- Fix schema for verify page functionality (Version 2)
...
-- Create a function to get trends for verification (with user info join)
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM public.trend_submissions LIMIT 1) THEN
        -- First ensure we have at least one user profile to use as spotter
        IF NOT EXISTS (SELECT 1 FROM public.user_profiles LIMIT 1) THEN
            -- Create a test user profile if none exist
            INSERT INTO public.user_profiles (id, email, username)
            VALUES (gen_random_uuid(), 'testuser@wavesight.com', 'testuser')
