-- Comprehensive fix for numeric field overflow
...
-- Recreate the public_trends view with BIGINT support
...
-- Recreate trends_for_validation view
...
-- Recreate available_trends_for_verification view
...
-- Add constraints to prevent overflow (though BIGINT is very large)
ALTER TABLE public.trend_submissions
...
DO $$
BEGIN
    RAISE NOTICE 'Numeric overflow fix completed successfully!';
    RAISE NOTICE 'All count fields now support values up to 9.2 quintillion.';
    RAISE NOTICE 'Views have been recreated with BIGINT support.';
END $$;