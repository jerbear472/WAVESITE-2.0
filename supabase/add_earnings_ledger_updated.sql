...
-- UPDATED: Removed scroll_session from earnings types
CREATE TABLE IF NOT EXISTS public.earnings_ledger (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,
    amount DECIMAL(10,2) NOT NULL,
    type TEXT NOT NULL CHECK (type IN ('trend_submission', 'trend_validation', 'challenge_completion', 'bonus', 'referral')),
    description TEXT,
    reference_id UUID, -- Links to trend_id, etc.
...
-- Function to calculate user stats (scroll sessions no longer generate earnings)
...
RETURNS TRIGGER AS $$
BEGIN
    -- Award $0.01 for validation (changed from $0.10)
    PERFORM add_earnings(
        NEW.validator_id,
        0.01,
        'trend_validation',
...
-- NOTE: Scroll sessions no longer trigger earnings
