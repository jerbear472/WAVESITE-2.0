-- Complete Payment System Fix for WaveSite
-- This creates all necessary tables and functions for the cashout system

-- 1. Create cashout_requests table
CREATE TABLE IF NOT EXISTS public.cashout_requests (
  ...
  transaction_id TEXT, -- External payment reference (Venmo, PayPal, etc)
  failure_reason TEXT,
  notes TEXT,
...
-- 3. Create payment_methods table for future expansion
CREATE TABLE IF NOT EXISTS public.payment_methods (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  type TEXT NOT NULL CHECK (type IN ('venmo', 'paypal', 'cashapp', 'zelle', 'bank')),
  account_identifier TEXT NOT NULL, -- username, email, or account number
...
-- 4. Add total_cashed_out column to profiles if it doesn't exist
DO $$
BEGIN
...
-- 5. Create function to update user's total_cashed_out
...
-- 6. Create trigger for updating cashed out totals
...
-- 7. Create function to get user's available balance
...
-- 8. Create view for user payment dashboard
...
-- 9. RLS Policies for cashout_requests
...
CREATE POLICY "users_create_own_cashouts" ON public.cashout_requests
  ...
  WITH CHECK (
    auth.uid() = user_id 
    AND amount >= 10.00 -- Minimum cashout amount
...
-- 12. Create function to validate cashout request
...
-- 13. Create admin view for processing payments
...
-- 14. Function to process cashout (for admin use)
...
-- 16. Sample data for testing (commented out - uncomment to test)
/*
-- Create a test cashout request
INSERT INTO public.cashout_requests (user_id, amount, venmo_username, status)
SELECT 
  id,
  25.00,
  '@testuser',
...
AND table_name IN ('cashout_requests', 'payment_methods')
...
SELECT 
  ...
  COUNT(*) as count
FROM pg_policies
WHERE tablename IN ('cashout_requests', 'payment_methods')
...
WHERE routine_name IN ('get_user_available_balance', 'validate_cashout_request', 'process_cashout_request');