-- Safe version of earnings system that checks for existing objects
-- This script can be run multiple times without errors

-- 1. Create earnings ledger table if it doesn't exist
...
-- 2. Create trend validations table if it doesn't exist
...
-- 3. Add earnings columns to profiles if they don't exist
DO $$ 
BEGIN
...
-- 4. Create or replace function to add pending earnings
...
-- 5. Create trigger for new submissions (drop if exists first)
...
-- 6. Create or replace function to check and approve earnings
...
-- 7. Create trigger for validation votes (drop if exists first)
...
-- 8. Create or replace view for user earnings summary
...
-- 10. Create RLS policies (drop existing ones first)
...
DROP POLICY IF EXISTS "System can create earnings" ON earnings_ledger;
...
DROP POLICY IF EXISTS "Users can create own validations" ON trend_validations;
CREATE POLICY "Users can create own validations" ON trend_validations
    FOR INSERT WITH CHECK (auth.uid() = validator_id);

DROP POLICY IF EXISTS "Users cannot validate own trends" ON trend_validations;
...
-- Add status column to trend_submissions if missing
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                  WHERE table_schema = 'public' 
...
-- Success message
SELECT 'Earnings system setup complete! Users earn $0.10 pending per submission, approved by majority vote.' as status;