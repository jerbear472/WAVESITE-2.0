-- FINAL EARNINGS FIX V2 - Run this in Supabase SQL Editor
-- Fixed to handle missing status column in earnings_ledger
...
-- 2. Check if earnings_ledger has status column, add if missing
DO $$
BEGIN
    IF EXISTS (
        ...
        IF NOT EXISTS (
            ...
            -- Set all existing entries to pending
            UPDATE earnings_ledger SET status = 'pending' WHERE status IS NULL;
        END IF;
    END IF;
END $$;

-- 3. Create or replace the add_pending_earnings function
...
BEGIN
    ...
    -- Add to earnings_ledger if it exists
    IF EXISTS (
        SELECT FROM information_schema.tables 
        WHERE table_name = 'earnings_ledger' 
        AND table_schema = 'public'
...
-- 5. Create trigger to auto-add earnings on trend submission
...
-- 7. Recalculate pending earnings (simplified - just sum all earnings_ledger entries)
DO $$
BEGIN
    -- Only recalculate if earnings_ledger exists and has a status column
    IF EXISTS (
        SELECT FROM information_schema.tables 
        WHERE table_name = 'earnings_ledger'
    ) THEN
        ...
        ELSE
            -- If no status column, just sum all entries as pending
            UPDATE user_profiles up
            SET pending_earnings = (
                SELECT COALESCE(SUM(amount), 0)
                FROM earnings_ledger
...
-- 8. Set some test earnings for current users (optional - comment out if not needed)
-- This gives all existing users a small pending balance to test with
UPDATE user_profiles 
SET pending_earnings = COALESCE(pending_earnings, 0) + 0.25
WHERE pending_earnings = 0 OR pending_earnings IS NULL
LIMIT 5;  -- Only update first 5 users for testing

-- 9. Test - Check if any users have earnings
SELECT 
...
BEGIN
    ...
    RAISE NOTICE '   2. The app reads from user_profiles table using id column';
    RAISE NOTICE '   3. Earnings will show in the UI immediately';
END $$;