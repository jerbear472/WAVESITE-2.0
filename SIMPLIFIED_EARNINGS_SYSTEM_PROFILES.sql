-- SIMPLIFIED EARNINGS SYSTEM (Using 'profiles' table)
-- This version uses 'profiles' table which seems to be what your database has
...
-- Ensure profiles table has all earnings columns
...
-- Ensure trend_submissions has all needed columns
...
-- Ensure trend_validations has reward columns
...
SELECT 
    ...
    p.earnings_approved,
    p.earnings_paid,
    (p.earnings_pending + p.earnings_approved) as earnings_available,
    p.total_earnings,
...
GROUP BY p.id, p.username, p.spotter_tier, p.earnings_pending, 
...
DO $$
BEGIN
    RAISE NOTICE '✅ SIMPLIFIED EARNINGS SYSTEM ACTIVE (profiles table version)';
    RAISE NOTICE '';
